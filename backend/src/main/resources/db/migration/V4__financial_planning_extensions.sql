CREATE TABLE financial_goal (
                                ...
                                monthly_contribution NUMERIC(19,2) CHECK (monthly_contribution IS NULL OR monthly_contribution >= 0),
                                target_date DATE,
...
CREATE TABLE excel_import_batch (
                                    ...
                                    detected_profile_type VARCHAR(30) CHECK (
    ...
    currency VARCHAR(3) NOT NULL DEFAULT 'ARS' CHECK (char_length(currency) = 3),
    summary_json JSONB,
    warnings_json JSONB,
    errors_json JSONB,
    created_at TIMESTAMP NOT NULL DEFAULT now(),
...
CREATE TABLE excel_import_row (
    ...
    amount NUMERIC(19,2),
    target_entity VARCHAR(40) CHECK (
        target_entity IN (
            'CATEGORY',
            'ACCOUNT',
            'INCOME',
    ...
    status VARCHAR(30) NOT NULL CHECK (
        status IN ('READY', 'IMPORTED', 'SKIPPED', 'WARNING', 'ERROR')
    ),
