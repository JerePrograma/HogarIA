...
class MonthlyPlanSuggestionServiceTest {
  ...
  @BeforeEach void setUp(){service=new MonthlyPlanSuggestionService(txRepo,itemRepo,accountRepo,categoryRepo,profileRepo); when(profileRepo.findByIdAndUserId(profileId,userId)).thenReturn(Optional.of(new FinancialProfile())); when(itemRepo.findAll()).thenReturn(new ArrayList<>());}
