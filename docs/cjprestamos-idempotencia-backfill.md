...
- `GET /external-loans/idempotency/diagnostics` resume candidatos CJ, mappings existentes, recomendación de backfill y duplicados que bloquearían los índices estrictos de V17.
...
- `hasIndexBlockingDuplicates`: existen filas actuales de `money_transaction` que violarían los índices únicos estrictos de V17 y deben resolverse antes de sincronizar.
...
Sirve para revisar duplicados bloqueantes, transacciones CJ sin mapping y mappings que apuntan a transacciones inexistentes antes o después de migraciones Flyway. No debe registrarse como migración.
