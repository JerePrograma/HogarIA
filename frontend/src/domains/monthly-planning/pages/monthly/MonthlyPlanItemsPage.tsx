...
import {
  parseSortKey,
  parseStatusFilterKey,
  parseTableFilterKey,
  parseTypeFilterKey,
} from '../../planningSearchParams';

export function MonthlyPlanItemsPage() {
  const {
    profileId,
    year,
    month,
  ...
  return (
    <MonthlyPlanItemsTable
      ...
      filter={parseTableFilterKey(params.get('filter'))}
      statusFilter={parseStatusFilterKey(params.get('status'))}
      typeFilter={parseTypeFilterKey(params.get('type'))}
      search={params.get('search') ?? ''}
      sortBy={parseSortKey(params.get('sort'))}
    />
  );
}
