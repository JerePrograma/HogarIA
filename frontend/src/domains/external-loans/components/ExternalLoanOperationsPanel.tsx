...
export function ExternalLoanOperationsPanel({
  ...
  const operationPending =
    dryRunPending || syncPending || diagnosticsPending || backfillDryRunPending || backfillApplyPending;
  ...
  const syncDisabled =
    operationPending || !hasSyncConfig || !canSync || readOnlyMode || syncBlockedByDiagnostics;
    ...
    <section className="panel">
      <div className="section-title">
        <div>
          ...
          <h2>CJPrestamos - Sincronizacion contable</h2>
        </div>
        <StatusBadge
      ...
      <div className="stack-ui">
        <section className="surface-inset">
          ...
          <div className="metric-grid">
            <article>
              ...
              <strong>{healthLoading ? 'Consultando...' : health?.message ?? 'Sin diagnostico'}</strong>
            </article>
            <article>
          ...
          {missingConfigItems.length > 0 ? (
            <p className="mensaje-warning mt-3">
              Configuracion incompleta: falta {missingConfigItems.join(', ')}.
            </p>
          ) : null}
        </section>
        ...
        <section className="surface-inset">
          ...
          {dryRunResult ? (
            <ExternalLoanSyncResultPanel title="Resultado de analisis sync" result={dryRunResult} />
          ) : (
            <p className="muted">Sin analisis de sync ejecutado.</p>
          )}
        ...
        <section className="surface-inset">
          ...
          {diagnostics ? (
            ...
            <p className="muted">Sin diagnostico de idempotencia ejecutado.</p>
          )}
          {diagnosticsError ? (
        ...
        <section className="surface-inset">
          ...
          <div className="page-actions">
            <button
              ...
              disabled={operationPending || !hasSyncConfig}
            >
            ...
            <button
              ...
              disabled={operationPending || safeBackfillCandidates.length === 0}
            >
...
function DuplicateGroupsSummary({
  ...
  if (sourceOperationGroups.length === 0 && sourceHashGroups.length === 0) {
    return <p className="mensaje-exito mt-3">Sin duplicados bloqueantes para los indices estrictos.</p>;
  }
