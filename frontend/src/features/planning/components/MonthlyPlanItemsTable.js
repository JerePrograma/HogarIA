...
export function MonthlyPlanItemsTable({ items, accounts, categories, onConvert, onCancel, onDelete, onUpdate, onMarkPaid, onMarkCollected, pendingActionId, actionError, profileId, externalFilterKey }) {
    ...
    const filtered = useMemo(() => items
        .filter((i) => (externalFilterKey === 'ALL' || !externalFilterKey || (externalFilterKey === 'UNPRICED' ? (i.amount == null && i.minAmount == null && i.maxAmount == null) : externalFilterKey === 'MISSING_CLASSIFICATION' ? (!i.transactionId && i.status !== 'CANCELLED' && (!i.accountId || !i.categoryId)) : externalFilterKey === 'READY_TO_CONVERT' ? canConvertPlanItem(i) : true)) &&
        (statusFilter === 'ALL' ||
