...
export function QuickPlanTextImportPage() {
  ...
  const preview = useMutation({
    mutationFn: () => previewQuickPlanText(profileId, { rawText: text, periodYear: year, periodMonth: month, defaultAmountScale: 'THOUSANDS' }),
    onSuccess: (d) => { setRows(d.candidates); setError(null); },
    onError: (e) => setError(getApiErrorMessage(e)),
  });

  const commit = useMutation({
    mutationFn: () => commitQuickPlanText(profileId, { periodYear: year, periodMonth: month, candidates: rows.map((r) => r.candidate), skipDuplicates: true }),
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: queryKeys.planning(profileId, year, month) }),
        qc.invalidateQueries({ queryKey: queryKeys.dashboard(profileId, year, month) }),
        qc.invalidateQueries({ queryKey: queryKeys.monthlyPlanReconciliation(profileId, year, month) }),
      ]);
    },
    onError: (e) => setError(getApiErrorMessage(e)),
  });
