...
export function formatPlanAmount(
  item: Pick<MonthlyPlanItem, 'amount' | 'minAmount' | 'maxAmount'>,
): string {
...
export function getPlanItemNextAction(item: MonthlyPlanItem): PlanItemNextAction {
  ...
  if (
    ['EXPENSE', 'DEBT', 'SAVING', 'TRANSFER'].includes(item.type) &&
...
): { label: string; className: string; tone: 'ok' | 'watch' | 'critical' } {
  if (confidence === 'HIGH') {
    return { label: 'Alta', className: 'badge-confidence-high', tone: 'ok' };
  }

  if (confidence === 'MEDIUM') {
    return { label: 'Media', className: 'badge-confidence-medium', tone: 'watch' };
  }

  return { label: 'Baja', className: 'badge-confidence-low', tone: 'critical' };
}