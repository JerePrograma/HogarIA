...
interface Props { rows: TransactionImportRow[]; categories: Category[]; onRowsChange: (rows: TransactionImportRow[]) => void; }
...
export function ImportRowsTable({ rows, categories, onRowsChange }: Props) {
  ...
  return <table><thead><tr><th>Fila</th><th>Fecha</th><th>Descripción</th><th>Monto</th><th>Tipo</th><th>Categoría</th><th>Estado</th></tr></thead><tbody>
    {rows.map((row, index) => <tr key={row.rowNumber}>
      ...
      {!row.suggestedCategoryId && row.suggestedCategoryName ? <small>Sugerida: {row.suggestedCategoryName} (se creará al confirmar)</small> : null}</td>
