...
export function CategoriesPage() {
  ...
  {!q.data?.length ? <p className='empty-state'>Sin categorías.</p> : <table className='table'><thead><tr><th>Nombre</th><th>Tipo</th><th>Alcance</th><th>Activa</th><th></th></tr></thead><tbody>{q.data.map((cat: any) => <tr key={cat.id}><td>{cat.name}</td><td>{typeLabels[cat.type] ?? cat.type}</td><td>{scopeLabels[cat.scope] ?? cat.scope}</td><td>{cat.active?'Sí':'No'}</td><td>{cat.scope==='GLOBAL' ? '-' : <><button onClick={()=>u.mutate(cat)}>Editar/Activar</button><button className='button-danger' onClick={()=>window.confirm('¿Desactivar categoría?') && d.mutate(cat.id)}>Desactivar</button></>}</td></tr>)}</tbody></table>}</div></AppLayout>;
}
